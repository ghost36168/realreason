...
------
    ...
    答案是可以，因為for x in xxx 就是將xxx，中的每個元素一一放進x，再執行下方程式碼，真正讓我覺得困擾的是
    做完之後我該如何儲存，可以用tony=dsf，方式存嗎?
    不行，但我又不會其它方式來一一儲存元素，最後構成list，所以我不用for 那改用while呢?
    應該可以
    while 要先有值在while外面
    n=0 
    ...
    因為alist[2]>alist[-1]
    跳出迴圈
    2.前面提到的問題導致這樣left=alist[i]永遠只會存一個數
    第一個問題:
    我是這樣解決的，為避免碰到前面就停下。我就採用if 讓所有比它大就到右邊去，這樣
